...
import type { Event, Task } from '../lib/database.types'
...
type AIResult = CategoriesTeamsResult | EventsResult | TasksResult | ShiftsResult
...
const CONTEXT_PLACEHOLDERS: Record<AIContext, string> = {
  ...
  tasks: 'esim. "Luo lipunmyynti ja järjestyksenvalvonta tehtävät"',
...
function loadMessages(context: AIContext): Message[] {
  try {
    ...
    return parsed.map((m: Message) => ({
      ...
      selectedTasks: m.selectedTasks ? new Set(m.selectedTasks as unknown as number[]) : undefined,
      selectedShifts: m.selectedShifts ? new Set(m.selectedShifts as unknown as number[]) : undefined,
    }))
  } catch { return [] }
}
...
export default function AdminAIAssistant({ context, onSaved, inline = false, eventId, taskId }: Props) {
  ...
  useEffect(() => {
    if (context === 'shifts' && selectedEventId) {
      supabase.from('tasks').select('id, name, event_id').eq('event_id', selectedEventId).order('name')
  ...
  async function handleSend() {
    ...
    try {
      ...
      const assistantMsg: Message = {
        ...
        selectedTasks: r.type === 'tasks' ? new Set(r.tasks.map((_, i) => i)) : undefined,
        selectedShifts: r.type === 'shifts' ? new Set(r.shifts.map((_, i) => i)) : undefined,
      }
  ...
  function toggleTask(msgIndex: number, idx: number) {
    setMessages(prev => prev.map((m, i) => {
      if (i !== msgIndex || !m.selectedTasks) return m
      const next = new Set(m.selectedTasks)
      next.has(idx) ? next.delete(idx) : next.add(idx)
      return { ...m, selectedTasks: next }
    }))
  }

  function toggleShift(msgIndex: number, idx: number) {
    setMessages(prev => prev.map((m, i) => {
      if (i !== msgIndex || !m.selectedShifts) return m
      const next = new Set(m.selectedShifts)
      next.has(idx) ? next.delete(idx) : next.add(idx)
      return { ...m, selectedShifts: next }
    }))
  }
  ...
  async function handleSave(msgIndex: number) {
    ...
    try {
      ...
      } else if (msg.result.type === 'events') {
      ...
      } else if (msg.result.type === 'tasks') {
        if (!selectedEventId) throw new Error('Valitse tapahtuma ensin')
        const tks = msg.result.tasks.filter((_, i) => msg.selectedTasks?.has(i))
        if (tks.length > 0)
          await supabase.from('tasks').insert(tks.map(t => ({
  ...
  function nothingSelected(msg: Message) {
    ...
    if (msg.result?.type === 'tasks')
      return (msg.selectedTasks?.size ?? 0) === 0
    if (msg.result?.type === 'shifts')
      return (msg.selectedShifts?.size ?? 0) === 0
    return true
  }
  ...
  // Context picker shown for tasks/shifts
  const contextPicker = (context === 'tasks' || context === 'shifts') && (
    <div className="border-b bg-gray-50 px-4 py-2 space-y-2">
      <div className="flex items-center gap-2">
        <label className="text-xs text-gray-500 w-20 shrink-0">Tapahtuma</label>
        <select
          value={selectedEventId}
          onChange={e => { setSelectedEventId(e.target.value); setSelectedTaskId('') }}
      ...
      {context === 'shifts' && (
        <div className="flex items-center gap-2">
          <label className="text-xs text-gray-500 w-20 shrink-0">Tehtävä</label>
          <select
            value={selectedTaskId}
            onChange={e => setSelectedTaskId(e.target.value)}
            disabled={!selectedEventId}
            className="flex-1 text-xs border border-gray-200 rounded-lg px-2 py-1.5 bg-white disabled:opacity-50"
          >
                    ...
                    <div className="max-w-[95%] space-y-2">
                      ...
                      {msg.result && (
                        <div className="bg-gray-50 border border-gray-200 rounded-2xl rounded-tl-sm p-3 space-y-3">
                          ...
                          {msg.result.type === 'tasks' && (
                            <div className="space-y-2">
                              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Tehtävät</p>
                              {msg.result.tasks.map((t, idx) => (
                                <label key={idx} className="flex items-start gap-2 cursor-pointer">
                                  <input
                                    type="checkbox"
                                    checked={msg.selectedTasks?.has(idx) ?? false}
                                    onChange={() => toggleTask(i, idx)}
                                    disabled={msg.saved}
                                    className="w-4 h-4 mt-0.5 rounded border-gray-300 text-indigo-600"
                                  />
                                  <div>
                          ...
                          {msg.result.type === 'shifts' && (
                            <div className="space-y-2">
                              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Vuorot</p>
                              {msg.result.shifts.map((s, idx) => (
                                <label key={idx} className="flex items-start gap-2 cursor-pointer">
                                  <input
                                    type="checkbox"
                                    checked={msg.selectedShifts?.has(idx) ?? false}
                                    onChange={() => toggleShift(i, idx)}
                                    disabled={msg.saved}
                                    className="w-4 h-4 mt-0.5 rounded border-gray-300 text-indigo-600"
                                  />
                                  <div>
                                    <p className="font-semibold text-sm text-gray-900">{formatTime(s.start_time)} – {formatTime(s.end_time)}</p>
                                    {s.location && <p className="text-xs text-gray-500">{s.location}</p>}
                                    <p className="text-xs text-gray-400">Max {s.max_participants} henkeä</p>
