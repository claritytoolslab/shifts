...
export const handler: Handler = async () => {
  try {
    // Hae vuorot jotka alkavat 23-25h päästä (jotta tunnin välein ajettaessa ei tule duplikaatteja)
    const now = new Date()
    ...
    for (const shift of shifts) {
      ...
      // Hae confirmed ilmoittautumiset tälle vuorolle
      ...
      for (const reg of registrations) {
        // Tarkista ettei muistutusta ole jo lähetetty (email_queue)
